...
// 假设你正在开发一款赛车游戏，游戏中有多个摄像机视角：
// - **主视角摄像机**：显示驾驶员眼中的前方路况。
// - **辅助视角摄像机**：显示车辆后方的情况，比如用于倒车辅助。
// - **子视角摄像机**：可能是用来展示车内的仪表盘或者车外环境的某个小窗口。
//
// 在这种场景下，每个摄像机都对应一个“视图（View）”。为了在渲染管线中高效地管理这些视图，
// 系统就会使用 RetainedViewEntity 来对它们进行标识和管理。
//
// 你可以把 RetainedViewEntity 想象为每个摄像机视图的“身份证”，它记录了以下关键信息：
// - 哪个是主视角（主摄像机实体）。
// - 哪些是辅助视角（辅助摄像机实体）。
// - 如果有多个子视图，还可以通过子视图索引来具体区分。
//
